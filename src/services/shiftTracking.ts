// src/services/shiftTracking.ts — DOT drive time shift bookend GPS capture
//
// Records login/logout GPS stamps to Firestore for DOT drive time auditing.
// Uses Firestore REST API (WB M doesn't have the Firestore JS SDK).
...
async function autoCloseStaleShift(
  ...
  for (let daysBack = 1; daysBack <= 3; daysBack++) {
    ...
    try {
      ...
      if (resp.status === 404) continue; // no doc for that day
      if (!resp.ok) continue;

      const doc = await resp.json();
      const eventsArray = doc.fields?.events?.arrayValue?.values || [];

      // Count logins vs logouts
      let logins = 0;
      let logouts = 0;
      ...
      if (logins > logouts) {
        // Unmatched login — auto-close at 23:59:59 of that day
      ...
      // Once we find a doc (open or closed), stop looking further back
      break;
    } catch (err) {
...
export async function recordShiftEvent(
  ...
  try {
    ...
    // On login, auto-close any stale open shift from previous days
    if (type === 'login') {
...
export async function checkShiftOnResume(
  ...
  try {
    ...
    // Check if today already has a doc
    const today = dateString(new Date());
    const path = docPath(driverId, today);
    const getUrl = `https://firestore.googleapis.com/v1/${path}?key=${FIREBASE_API_KEY}`;

    const resp = await fetch(getUrl);
    if (resp.ok) return; // today's doc exists already

    // No doc for today — record a login
    const gps = await captureGPS();
    if (!gps) return;
